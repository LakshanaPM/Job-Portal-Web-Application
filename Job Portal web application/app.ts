...
let appliedJobIds: number[] = [];  // NEW: track applied jobs
let currentPage = 1;
const jobsPerPage = 6;
...
// DOM elements (with safe casting)
...
const savedJobsList = document.getElementById("savedJobsList") as HTMLElement;
const savedCountSpan = document.getElementById("savedCount") as HTMLElement;
const clearSavedBtn = document.getElementById("clearSavedBtn") as HTMLButtonElement;
...
const applyModal = document.getElementById("applyModal") as HTMLElement;
const modalJobTitle = document.getElementById("modalJobTitle") as HTMLElement;
const modalCompanyName = document.getElementById("modalCompanyName") as HTMLElement;
...
const cancelModalBtn = document.querySelector(".cancel-modal-btn") as HTMLElement;
...
async function fetchMockJobs(): Promise<Job[]> {
...
function updateJobCardsBookmarkState() {
  document.querySelectorAll(".save-btn").forEach(btn => {
    const jobId = parseInt((btn as HTMLElement).getAttribute("data-id")!);
...
function renderJobs() {
  ...
  // Attach event listeners for apply and save buttons
...
// Modal handlers
let currentApplyJob: Job | null = null;
...
function openApplyModal(job: Job) {
  ...
  (document.getElementById("applicantName") as HTMLInputElement).value = "";
  (document.getElementById("applicantEmail") as HTMLInputElement).value = "";
}
...
applyForm.addEventListener("submit", (e) => {
  e.preventDefault();
  const name = (document.getElementById("applicantName") as HTMLInputElement).value.trim();
  const email = (document.getElementById("applicantEmail") as HTMLInputElement).value.trim();
  if (!name || !email) {
